#!/usr/bin/env python3
"""
Adds the fully vectorized predictive array mapping method to the Decision Tree.
"""
import numpy as np
...
class Node:
    ...
    def update_indicator(self):
        """Initializes indicator logic checking whether features satisfy node bounds."""
    ...
    def pred(self, x):
        """Slower element-wise baseline traversal route."""
...
class Leaf(Node):
    ...
    def pred(self, x):
        """Returns the raw assignment output value."""
        return self.value
...
class Decision_Tree():
    ...
    def update_predict(self):
        """Vectorized execution approach mapping multi-dimensional observations to leaf elements."""
        self.update_bounds()
        leaves = self.get_leaves()
        for leaf in leaves:
            leaf.update_indicator()
        
        self.predict = lambda A: np.dot(np.array([leaf.indicator(A) for leaf in leaves]).T,
                                        np.array([leaf.value for leaf in leaves]))

    def pred(self, x):
        """Element-wise classification route."""
